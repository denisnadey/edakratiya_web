...
@Component(
  selector: 'categorymenu_state_component',
  templateUrl: 'categorymenu_state_component.html',
  directives: [
...
class CategoryMenuStateComponent implements AfterViewInit {
  @Input()
  DishesState state;
  ...
  List<Category> get categories => isSuccess
      ? (state as LoadedDishesState)
          ...
          .toList()
      : [];
  String get error => isError
      ? ((state as ErrorDishesState).failure.error?.stackTrace?.toString() ??
          '')
      : '';
  @override
  void ngAfterViewInit() {
    js.context.callMethod('startCategoryMenu');
  }
}
